...
# from xtuner.model.modeling_llava import CustomLlavaNextForConditionalGeneration, ResnetExpertModel
# from xtuner.model.processing_llava_next import CustomLlavaNextProcessor

from swift.new_models.config_llava_new import LlavaWithVisionExpertConfig, ResnetConfig
from swift.new_models.modeling_llava import CustomLlavaNextForConditionalGeneration, ResnetExpertModel
...
class ModelType:
    # qwen
    qwen_1_8b = 'qwen-1_8b'
    qwen_1_8b_chat = 'qwen-1_8b-chat.5'
...
def get_model_tokenizer_llava_hf(model_dir: str, *args, **kwargs):
    ...
    # new_tokens = ["<REAL>", "<FAKE>"]
    # tokenizer.add_special_tokens({"additional_special_tokens": new_tokens})
    # model.resize_token_embeddings(len(tokenizer))
    #
    # with torch.no_grad():
    #     new_embeddings = model.get_input_embeddings().weight[-len(new_tokens):]
    #     new_embeddings.copy_(torch.mean(model.get_input_embeddings().weight[:-len(new_tokens)], dim=0))
...
def get_model_tokenizer_llava_next(*args, **kwargs):
    from transformers import LlavaNextForConditionalGeneration, AutoModel
    if 'automodel_class' not in kwargs:
